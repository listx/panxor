...
progOpts = Opts
	{ hex = [] &= name "x" &= typ "HEXADECIMAL" &= help
		"Additional hashes in hexadecimal to XOR into (use this flag once for"
		++ " each additional hash; also, do not prefix the hex with `0x'; e.g.,"
		++ " use `f3' instead of `0xf3'). Leading zeroes are ignored; trailing"
		++ " non-hex characters (as well as non-leading-hex strings) are also"
		++ " ignored."
	...
	, stdin_hex = False &= help
		"Enable reading from STDIN. Only hexadecimal values (sha1sum(1) format)"
		++ " are read in with this option. If no input files are specified with"
		++ " --file-{hex,dec,bin}, and no other hashes are specified with"
		++ " --{hex,dec,bin}, then this flag is automatically turned on. In"
		++ " other words, if no arguments are specified, then panxor expects"
		++ " input from STDIN."
	, out_hex = False &= help "Output the final hash in hexadecimal (without"
		++ " the leading `0x'). If no output format is specified with"
		++ " --out-{hex,dec,bin}, then this flag is turned on automatically."
	...
	, min_length = 0 &= help
		"Output length must be at least INT long (if output is lesser than this"
	++ " length, then the leading digits are padded with 0). Default is 0."
	}
	&= details
		[ "Notes:"
		, ""
